...
# メッセーじが削除サッれているか確認するためにcreate_listを用いて、メッセージと紐づいた、ユーザーとチャットルームを中間テーブルに生成している。 

# 「チャットを終了する」を取得してクリックするための指示を記述。クリックするとチャットルームは削除され、同じタイミングでmessesモデルのカウントが５つ減っていることを確認する。